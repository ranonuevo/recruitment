...
import * as testFns from './testFns'
...
const getEquivalentTest = (
    ...
    switch (testType) {
      ...
      case 'email-addresses':
        return {
          ...
          function: testFns.testEmailAddresses
        }
      default:
  ...
  if (fieldType === 'boolean') {
    switch (testType) {
      case 'must-boolean-true':
        return {
          name: 'boolean-must-boolean-true',
          messages: params[1]?? 'Mandatory field',
          function: testFns.testIsBoolTrue
        }
      default:
        return validationType
    }
...
export const jsonToYupSchema = (fields, formValues) => {
  fields = fields
    ...
    .map(f => {
      ...
      if ('disabled' in fieldProps) {
        if (typeof fieldProps.disabled === 'function') {
          isFieldDisabled = fieldProps.disabled(formValues,f.name)
        } else {
          isFieldDisabled = fieldProps.disabled
        }
      }

      if (f.mandatoryWhen && f.mandatoryWhen(formValues, f.name) === false || isFieldDisabled) {
